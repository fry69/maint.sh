...
for ((i=0; i<${#commands[@]}; i++)); do
    case "${commands[i]}" in
        res)
            ...
            ;;
        upd)
            remote "uname -a && date && sudo LC_ALL=C DEBIAN_FRONTEND=noninteractive apt-get -q update && sudo LC_ALL=C DEBIAN_FRONTEND=noninteractive apt-get -q --allow-downgrades --allow-remove-essential --allow-change-held-packages -y dist-upgrade"
            ;;
        rbt)
